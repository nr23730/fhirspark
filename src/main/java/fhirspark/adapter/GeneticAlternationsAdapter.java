...
public class GeneticAlternationsAdapter {
    ...
    public Resource process(GeneticAlteration geneticAlternation) {
        ...
        variant.setMeta(new Meta()
                .addProfile("http://hl7.org/fhir/uv/genomics-reporting/StructureDefinition/variant"));
        ...
        variant.setCode(new CodeableConcept(
                new Coding("http://loinc.org", "69548-6", "Genetic variant assessment")));
        variant.getValueCodeableConcept().addCoding(new Coding("http://loinc.org", "LA9633-4", "Present"));

        if (geneticAlternation.getChromosome() != null && !geneticAlternation.getChromosome().equals("NA")) {
            ObservationComponentComponent chromosome = new ObservationComponentComponent()
                    .setCode(new CodeableConcept(new Coding("http://loinc.org", "48001-2",
            ...
            chromosome.getValueCodeableConcept()
                    .addCoding(new Coding().setSystem(ChromosomeHuman.NULL.getSystem())
                            .setCode(geneticAlternation.getChromosome()));
            variant.addComponent(chromosome);
        }

        ObservationComponentComponent hgvsp = new ObservationComponentComponent().setCode(new CodeableConcept(
                new Coding("http://loinc.org", "48005-3", "Amino acid change (pHGVS)")));
        hgvsp.getValueCodeableConcept().addCoding(new Coding().setSystem("http://varomen.hgvs.org")
                .setCode("p." + geneticAlternation.getAlteration()));
        variant.addComponent(hgvsp);

        ObservationComponentComponent ncbiGeneId = new ObservationComponentComponent()
                .setCode(new CodeableConcept(
                        new Coding("http://loinc.org", "81252-9", "Discrete genetic variant")));
        ncbiGeneId.getValueCodeableConcept()
                .addCoding(new Coding().setSystem("http://www.ncbi.nlm.nih.gov/clinvar")
        ...
        ObservationComponentComponent hgnc = new ObservationComponentComponent().setCode(
                new CodeableConcept(new Coding("http://loinc.org", "48018-6", "Gene studied [ID]")));
        hgnc.getValueCodeableConcept().addCoding(
                new Coding("http://www.genenames.org/geneId", gn.getHgncId(), gn.getApprovedSymbol()));
